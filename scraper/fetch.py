    ...
    async def run(self):
        ...
        async with async_playwright() as pw:
            ...
            for i, (r, name, source) in enumerate(person_records[:50]):
                info = await _lookup_dcad(context, name)
                for f in ["prop_address","prop_city","prop_zip","mail_address","mail_city","mail_state","mail_zip"]:
                    if not r.get(f) and info.get(f):
                        r[f] = info[f]
                # Store the person name we actually looked up
                if source == "grantee" and not r.get("person_name"):
                    r["person_name"] = name
                log.info(f"  DCAD {i+1}/{min(len(person_records),50)}: '{name}' ({source}) → addr='{r.get('prop_address','')}' city='{r.get('prop_city','')}' zip='{r.get('prop_zip','')}'")
                await asyncio.sleep(0.5)
